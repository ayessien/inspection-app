...
import { getCommissioningInspection } from "../graphql/queries";
import { updateCommissioningInspection } from "../graphql/mutations";
const client = generateClient();
export default function CommissioningInspectionUpdateForm(props) {
  const {
    id: idProp,
    commissioningInspection: commissioningInspectionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = commissioningInspectionRecord
      ? { ...initialValues, ...commissioningInspectionRecord }
      : initialValues;
    setSiteId(cleanValues.siteId);
    setPhase(cleanValues.phase);
    setEquipmentTag(cleanValues.equipmentTag);
    setChecklistItem(cleanValues.checklistItem);
    setStatus(cleanValues.status);
    setTechAlias(cleanValues.techAlias);
    setNotes(cleanValues.notes);
    setInspectionDate(cleanValues.inspectionDate);
    setErrors({});
  };
  const [commissioningInspectionRecord, setCommissioningInspectionRecord] =
    React.useState(commissioningInspectionModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getCommissioningInspection.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getCommissioningInspection
        : commissioningInspectionModelProp;
      setCommissioningInspectionRecord(record);
    };
    queryData();
  }, [idProp, commissioningInspectionModelProp]);
  React.useEffect(resetStateValues, [commissioningInspectionRecord]);
  const validations = {
    siteId: [],
    phase: [],
    equipmentTag: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateCommissioningInspection.replaceAll("__typename", ""),
            variables: {
              input: {
                id: commissioningInspectionRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "CommissioningInspectionUpdateForm")}
      {...rest}
    >
      <TextField
        label="Site id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || commissioningInspectionModelProp)}
